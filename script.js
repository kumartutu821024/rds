...
const BASE_API = "https://cds-journey-api-gules.vercel.app";
const NEW_BASE_API = "https://cds-api-new.vercel.app/API";
...
async function loadCourses() {
  ...
  try {
    ...
    const [oldRes, newRes] = await Promise.allSettled([
      fetch(`${BASE_API}/all-batches`),
      fetch(`${NEW_BASE_API}/batches`)
    ]);
...
async function loadSubjects(courseId) {
  ...
  if (courseId.startsWith('new_')) {
    ...
    try {
      const response = await fetch(`${NEW_BASE_API}/batch/${rawId}`);
  ...
  try {
    const response = await fetch(`${BASE_API}/subjects/${courseId}`);
    if (!response.ok) {
...
async function loadContent(contentId) {
  ...
  try {
    const response = await fetch(`${BASE_API}/subject/content/${contentId}`);
    if (!response.ok) {
      throw new Error(`Server returned status ${response.status}`);
    }
    const json = await response.json();
  ...
  grid.innerHTML = filtered.map(course => {
    ...
    return `
      <div class="course-card" id="course-card-${course.id}">
        <span class="course-badge">${course.filterType === 'premium' ? 'Premium Batch' : 'Free Resource'}</span>
